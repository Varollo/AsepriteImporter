...
namespace Varollo.AsepriteImporter
{
    ...
    public class AsepriteSheet
    {
        ...
        public IEnumerable<AsepriteFrame> this[string name] => FrameNameMap.TryGetValue(name, out var frame) ? frame : Array.Empty<AsepriteFrame>();
        ...
        private void InitializeMapByTagData(AsepriteFrame[] frames, AsepriteTagData[] tags)
        {
            ...
            while (frameList.Count > 0)
                foreach (var tag in tags)
                {
                    ...
                    if (tag.Direction is AsepriteAnimationDirection.PingPong or AsepriteAnimationDirection.PingPongReverse)
                        loopFrameCount *= 2;
